...
const FooteFrame = () => {
	...
	return (
		<div className={`w-full h-auto py-10 px-5 bg-slate-950`}>
			<div
				className={`container grid grid-cols-3 gap-5 max-lg:grid-cols-1 max-lg:gap-10`}
			>
				<div
				...
				>
					...
					>
						<div
							className={`w-full h-auto flex justify-start items-start gap-2 hover:text-highlight`}
						>
				...
				>
					...
					>
						<InputComponent
							label={dictionary?.footer.email || "email"}
							type="text"
							background="bg-slate-200"
						/>
						<InputComponent
							label={
								dictionary?.footer.description || "description"
							}
							type="textarea"
							background="bg-slate-200"
						/>
						<button
							className={`w-full h-auto px-4 py-2 bg-gradient-to-br from-from via-via to-to rounded text-sm font-bold`}
						>
